...
class LoginResponse(BaseSchema):
    access_token: str = Field(
        ..., description="JWT token that could be used on Authorization header"
    )
...
class LoginController(ControllerBase):
    ...
    def run(cls, payload: request_model) -> response_model:
        if (
            payload.name in Config.auth_admins
            and payload.password == Config.auth_admins[payload.name]
        ):
            return cls.response_model(
                **{
                    "access_token": JWTAuthenticator.dump_access_token(
        ...
        if (
            payload.name in Config.auth_users
            and payload.password == Config.auth_users[payload.name]
        ):
            return cls.response_model(
                **{
                    "access_token": JWTAuthenticator.dump_access_token(
